        ...
        const checkFavorites = async () => {
            ...
            for (const [partOfSpeech, entry] of Object.entries(groupedEntries)) {
                ...
                entry.senses.forEach(async (_, j: number) => {
                    ...
                    if (Object.keys(favs).length === Object.values(groupedEntries).reduce((a: number, e) => a + e.senses.length, 0)) {
                        setFavorites({ ...favs });
                    }
                });
            }
                                            ...
                                            <ActionPanel>
                                                ...
                                                <Action
                                                    ...
                                                    onAction={async (): Promise<void> => {
                                                        if (!isFavorite) {
                                                            Favorite.addEntry(languageFull, word, sense.markdown || "", entryURL || "", j, partOfSpeech);
                                                            await showToast({
