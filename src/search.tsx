import { LaunchProps, ActionPanel, Action, Icon, Alert, confirmAlert, showToast, Toast, open, Clipboard, showHUD, List, Color } from "@raycast/api";
...
export default function Command(props: LaunchProps<{ arguments: { word: string; language: string } }>): JSX.Element {
    ...
    return (
        <List
            isLoading={Object.keys(groupedEntries).length === 0}
        ...
        >
            {
                !Object.keys(groupedEntries).length || Object.keys(groupedEntries).every(partOfSpeech => groupedEntries[partOfSpeech].senses.every(sense => !sense.definition.toLowerCase().includes(searchText.toLowerCase())))
                    ? (
                                        ...
                                        <List.Item
                                            ...
                                            detail={
                                                <List.Item.Detail markdown={
                                                    sense.markdown || "No details available."
                                                }
                                                />
                                            }
                                            actions={
